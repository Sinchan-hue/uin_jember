    ...
    switch ($action) {
        case 'delete':
            ...
            if ($koneksi->query($query)) {
                ...
                echo "<script>alert('Gagal menghapus data mahasiswa.');</script>";
            }
            break;
    }
}
...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $query .= " WHERE nama LIKE '%$search%' OR nim LIKE '%$search%' OR program_studi LIKE '%$search%'";
}
        ...
        <div class="container">
            ...
            <div class="search-and-add">
                ...
                <form method="GET" action="" class="search-form">
                    <input type="text" name="search" placeholder="Cari nama, NIM, atau program studi..." value="<?php echo isset($_GET['search']) ? $_GET['search'] : ''; ?>">
                    <button type="submit"><i class="fas fa-search"></i> Cari</button>
                </form>

                <!-- Tombol Tambah Data -->
                <a href="form_mahasiswa.php" class="btn-tambah"><i class="fas fa-plus"></i> Tambah Data</a>
            </div>
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            echo "<tr>
                                    ...
                                    <td>{$row['telepon']}</td>
                                    <td>
                                        <a href='form_mahasiswa.php?id={$row['id_mahasiswa']}' class='btn-edit'><i class='fas fa-edit'></i> Edit</a>
                                        <a href='crud_mahasiswa.php?action=delete&id={$row['id_mahasiswa']}' class='btn-hapus' onclick='return confirm(\"Apakah Anda yakin ingin menghapus data ini?\");'><i class='fas fa-trash'></i> Hapus</a>
                                    </td>
                                  </tr>";
                            $no++;
                        }
